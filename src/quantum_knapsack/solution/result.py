...
class Result:
    ...
    def __init__(
            ...
            states: Sequence[NDArray[np.complex128]],
            initial_h_strength: float,
            energy_scale: float,
            num_points: int
    ) -> None:
        ...
        self._problem_params = ProblemParameters(
            weights=mapping.knapsack.weights,
            profits=mapping.knapsack.profits,
            capacity=mapping.knapsack.capacity,
            penalty_scale=mapping.penalty_scale
        )

        # Store algorithm parameters
        self._algorithm_params = AlgorithmParameters(
            initial_h_strength=initial_h_strength,
    ...
    def _compute_probabilities(self, num_points: int) -> None:
        ...
        self._probabilities = np.zeros((len(indices), dim))

        for idx, t_idx in enumerate(indices):
            state = self._states[t_idx]
            self._probabilities[idx] = np.real(state.conj() * state).flatten()

    def _compute_expectation_values(self) -> None:
    ...
    def _compute_eigenspectrum(self) -> None:
        ...
        self._eigenspectrum = [
            obs.eigenvalues for obs in self._observables
        ]
    ...
    def get_correct_solutions(
        ...
        return {
            idx: prob for idx, prob in enumerate(final_probs)
    ...
    @property
    def eigenspectrum(self) -> Sequence[Tuple[float, NDArray[np.complex128]]]:
        """Get energy eigenspectrum.
        
        Returns:
            Sequence[Tuple[float, NDArray[np.complex128]]]: 
                Eigenvalues and eigenvectors
        """
        return self._eigenspectrum.copy()
